...
function detectStrategy(trades) {
	...
	try {
		...
		const oppositeDirections =
			(trade1.side === 'buy' && trade2.side === 'sell') || (trade1.side === 'sell' && trade2.side === 'buy');
...
function calculateMetrics(trades) {
	try {
		const expiryDiff = Math.round(
			(new Date(trades[0].expiration_date) - new Date(trades[1].expiration_date)) / (1000 * 60 * 60 * 24),
		);

		const netPremium = trades.reduce((sum, trade) => sum + (trade.side === 'sell' ? trade.premium : -trade.premium), 0);
...
• Price per contract: $${trade.price}`,
	)
	.join('\n')}

Position Metrics:
• Net Premium: ${
			metrics.netPremium !== 'N/A'
				? (metrics.netPremium > 0 ? '+$' : '-$') + Math.abs(metrics.netPremium).toFixed(2)
				: 'N/A'
		}
			...
			messages: [
				{
					role: 'system',
					content:
						'You are an elite institutional crypto derivatives trader specializing in complex options strategies and volatility trading. Provide sophisticated analysis with emphasis on practical trading implications and risk management.',
				},
				{
					role: 'user',
					content: prompt,
...
router.post('/analyze-metrics', async (req, res) => {
	try {
		...
		const total =
			parseFloat(dm.Call_Buys) + parseFloat(dm.Call_Sells) + parseFloat(dm.Put_Buys) + parseFloat(dm.Put_Sells);

		const prompt = `Analyze the following ${metrics.asset} options market activity on ${getExchangeName(
			exchange,
...
2. Top Traded Options Analysis:
   ${popularOptions.rows
			.slice(0, 5)
			.map((row) => `• ${row.instrument_name}: ${row.trade_count} trades`)
			.join('\n   ')}
			...
			messages: [
				{
					role: 'system',
					content:
						'You are a crypto options market analyst specializing in derivatives exchanges. Provide clear, actionable insights about market activity and potential implications. Use data-driven analysis to support your conclusions.',
				},
				{
					role: 'user',
					content: prompt,
...
function formatHourlyDistribution(timeDistribution) {
	...
	return Object.entries(hourlyData)
		...
		.map(([hour, data]) => `• ${String(hour).padStart(2, '0')}:00 UTC: ${data.calls} calls, ${data.puts} puts`)
		.join('\n   ');
}
function formatExpirationActivityForPrompt(expirationData) {
	...
	return sortedDates
		.slice(0, 5)
		.map(({ date, total, ratio }) => `• ${date}: ${total} trades (C/P ratio: ${ratio.toFixed(2)})`)
		.join('\n   ');
}
...
function formatStrikeActivityForPrompt(strikeData) {
	...
	return Object.entries(aggregated)
		.sort(([, a], [, b]) => b - a)
		.slice(0, 5)
		.map(([strike, count]) => `• Strike ${strike}: ${count} trades`)
		.join('\n   ');
}
